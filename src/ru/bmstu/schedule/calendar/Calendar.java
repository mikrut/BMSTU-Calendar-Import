...
public class Calendar {
	...
	private static String[] CALENDAR_PROJECTION = { Calendars.NAME,
			Calendars.VISIBLE, Calendars.SYNC_EVENTS, Calendars.ACCOUNT_NAME,
			Calendars.ACCOUNT_TYPE, Calendars.CALENDAR_COLOR,
			Calendars.CALENDAR_ACCESS_LEVEL, Calendars.OWNER_ACCOUNT,
			Calendars._ID };
	...
	private void update(ContentValues values) {
		Uri calUri = Calendars.CONTENT_URI
				...
				.appendQueryParameter(CalendarContract.Calendars.ACCOUNT_TYPE,
						"com.google").build();
		String query = "(" + Calendars.NAME + " = ?) AND ("
	...
	public static int deleteByName(String name, String account,
	...
	public static Calendar findByName(String name, ContentResolver resolver) {
		...
		Cursor cur = resolver.query(Calendars.CONTENT_URI, CALENDAR_PROJECTION,
				query, mSelectionArgs, null);
		if (null == cur || cur.getCount() <= 0)
			return null;
		cur.moveToFirst();
		Calendar cal = new Calendar(cur.getString(PROJECTION_NAME_INDEX),
				cur.getString(PROJECTION_ACCOUNT_NAME_INDEX), resolver)
				.setVisible(cur.getString(PROJECTION_VISIBLE_INDEX).equals("1"))
				.setSyncEvents(
						cur.getString(PROJECTION_SYNC_EVENTS_INDEX).equals("2"))
	...
	public String toString() {
		return new StringBuilder().append(" ~=~ ").append(name)
				.append(" ~=~ \n").append("Owner: ").append(ownerAccount)
				.append('\n').append("Rights: ").append(accessLevel)
				.append('\n').append("Account type: ").append(accountType)
				.append('\n').append("Visibility: ").append(visible)
				.append('\n').append("Sync Events: ").append(syncEvents)
				.append('\n').append("Acc name: ").append(accountName)
				.append('\n').toString();
	}
